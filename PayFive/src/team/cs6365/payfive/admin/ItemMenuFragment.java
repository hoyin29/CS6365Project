...
public class ItemMenuFragment extends Fragment {
	...
	private void addItemsToDB(int n) {
		...
		ds.create();
		Random r = new Random();
		//Bitmap bmap = BitmapFactory.decodeResource(getResources(), R.drawable.placeholder);
		for(int i = 0; i < n; ++i) {
			Log.d(TAG, "adding item" + i);
			ds.addMenuItem("item" + i, r.nextDouble() * 10 + 1, "category" + i, "description" + i, "");
		}
		ds.close();
	}
	...
	private void getAllItemsFromDB() {
		...
		Toast.makeText(ctx, "retrieved " + items.size() + " items from DB", Toast.LENGTH_LONG).show();
	...
	private void actionBarCustomerView() {
		...
		fragmentManager.beginTransaction()
				.replace(R.id.content_frame, customerViewFragment)
				.commit();
	}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_PICTURE && resultCode == ctx.RESULT_OK && data != null) {
	...
	private void contextMenuView(final int pos) {
		...
		MenuItemDataSource ds = new MenuItemDataSource(ctx);
		ds.open();
		curr = ds.getMenuItem(curr.getName(), curr.getPrice(), curr.getCategory(), curr.getDescription());
		items.set(pos, curr);
		ds.close();
		...
		final EditText etDesc = (EditText) dialog.findViewById(R.id.etDescription);
		...
		if(curr.getThumbnail().equals("")) {
			...
			ivPic.setImageDrawable(getResources().getDrawable(R.drawable.placeholder));
		}
		else {
	...
	private void contextMenuEdit(final boolean isEdit, final int pos) {
		...
		bSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Item i = new Item(etName.getText().toString(),
					Double.valueOf(etPrice.getText().toString()),
					etCate.getText().toString(),
